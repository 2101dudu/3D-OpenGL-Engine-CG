...
def create_moon(moon_name, model_file):
    # Generates a random position for the moon around the planet:
...
def main():
    ...
    sun_transform = {"rotate": None, "translate": None, "scale": {"x": 3, "y": 3, "z": 3}}
    ...
    for planet, distance, scale in planet_data:
        # Orbital distribution: uses a random angle
        angle = random.uniform(0, 360)
        transform = {
            "rotate": {"angle": angle, "x": 0, "y": 1, "z": 0},
        ...
        planet_group = create_group(planet, transform, "../../objects/sphere.3d")
        # Adds moons randomly
        for i in range(moons_per_planet[planet]):
            moon = create_moon(f"{planet}_Moon_{i+1}", "../../objects/sphere.3d")
            planet_group.append(moon)
        solar_system.append(planet_group)
    
    # Inner asteroid belt (farther from Mars to avoid overlap)
    add_asteroid_belt(solar_system, num_asteroids=1000, min_dist=32, max_dist=38)
    # Outer asteroid belt (closer to the Sun)
    add_asteroid_belt(solar_system, num_asteroids=1000, min_dist=120, max_dist=140)
